# Knowledge Platform Use Case Specifications (유스케이스 세부 명세서)

> **Document ID**: `0816_usecase_spec`  
> **Date**: 2026-08-16  
...
> **Status**: Approved Reference Specification (Final Specification)  

---

## 1. 개요 (Overview)

본 문서는 Knowledge Platform의 11가지 핵심 유스케이스(UC-001 ~ UC-011)에 대한 상세 표준 명세서입니다. 
지식의 수집(Ingestion), 구조화(Structure), 영속화(Storage), 제어 UI(Management), 플랫폼 독립성(Platform), 에이전트 인프라(Agent), 지식 제공(Retrieval), 거버넌스(Governance), 승인 관문(Approval), 수명주기 관리(Lifecycle), 외부 시각화 연동(Exploration)의 11개 세부 영역별로 기능 및 표준 규격을 정의합니다.

> [!NOTE]
> 본 명세서의 추적성(Traceability: `REQ`, `DSGN`, `TEST`) 및 비기능 요구사항(`NFR`) 세부 코드는 [.doc/0816_requirements_and_nfr_spec.md](file:///home/chaehwan/bifacewiki/bifacewiki/.doc/0816_requirements_and_nfr_spec.md)와 연동되며, 시스템 전체 구조는 [.doc/0816_knowledge_platform_architecture_spec.md](file:///home/chaehwan/bifacewiki/bifacewiki/.doc/0816_knowledge_platform_architecture_spec.md)를 참조합니다.

---

## 2. 유스케이스 상세 명세서 (UC-001 ~ UC-011)
...
| **규칙** | **Business Rule** | **BR-001-1**: 파일 1개당 반드시 1개의 원자적(Atomic) 주제만 다루어야 함 (Atomic Knowledge Pattern).<br/>**BR-001-2**: 실패 사례(Negative Knowledge)는 삭제하거나 은폐하지 않고 `type: negative_knowledge` 노드로 독립시켜 LLM 재발 방지 레퍼런스로 활용함. |
...
## 3. 추적성 매트릭스 및 유스케이스 요약 (Summary Matrix)

| UC ID | Use Case Name | 핵심 영역 | 핵심 가치 및 차별화 요소 | 연관 REQ Tag |
| :--- | :--- | :--- | :--- | :--- |
| **UC-001** | Q&A 지식 추출 및 저장 | Ingestion | **Negative Knowledge (실패 경험/하지 말아야 할 동작)** 수집 및 Minimal Frontmatter 3대 단순 규칙 적용 | `REQ-INGEST-01`, `REQ-INGEST-02` |
| **UC-002** | Node & Edge 체계적 분류 | Structure | Zettelkasten 원자적 마크다운 노드 및 본문 `[[Wikilinks]]` 기반 **Ref-DAG 인덱서** 구성 | `REQ-GRAPH-01`, `REQ-GRAPH-02` |
| **UC-003** | Git Repository 저장 및 관리 | Storage | DB 없는 File-based Git 영속화, 원자적 이력 관리, Diff 비교 및 롤백 편의성 | `REQ-STORAGE-01`, `REQ-STORAGE-02` |
| **UC-004** | Git 생성 및 관리를 위한 UI | Management | CLI 없는 대시보드 중심의 저장소 생성, Sync(Push/Pull), 이력 뷰어 GUI 제공 | `REQ-UI-GIT-01`, `REQ-UI-GIT-02` |
| **UC-005** | Vendor-Agnostic 지식 유지 | Platform | LLM 공급자(OpenAI, Gemini, Local) 변경에도 영향을 받지 않는 단일 마크다운 지식 창구 보장 | `REQ-PLATFORM-01` |
| **UC-006** | Skill & Prompt 원클릭 주입 UI | Agent | 대화창 텍스트 오염 없이 UI 클릭 한 번으로 `.agent/` 프롬프트/스킬을 LLM 런타임에 주입 | `REQ-AGENT-BIND-01` |
| **UC-007** | Agent/Skill 기반 LLM 지식 제공 | Retrieval | RAG 하드코딩 대신 Agent가 제공하는 표준 Skill(`knowledge_search/retrieve/inject`)로 동적 지식 전달 | `REQ-RETRIEVAL-01` |
| **UC-008** | 지식 정합성 검사 및 자동 린팅 | Governance | Broken Link, Orphan Node, Contradiction, Stale Node를 24/7 탐지하는 지식 린터 | `REQ-GOV-LINT-01` |
| **UC-009** | 인간 검증 및 승인 관문 | Approval | AI 지식의 무단 프로덕션 직접 반영 차단 (`draft` -> **Human Broker Approval** -> `main`) | `REQ-GOV-APPROVE-01` |
| **UC-010** | 지식 병합 및 폐기 수명주기 | Lifecycle | 유사 노드 통합(Merge), 낡은 지식 Archive/Prune, 안티패턴 변환 수명주기 관리 | `REQ-LIFECYCLE-01` |
| **UC-011** | 외부 그래프 뷰어 툴 연동 | Exploration | 뷰어 개발 오버헤드 최소화, **Obsidian/Logseq 원클릭 연동**으로 전문 2D/3D 그래프 탐색 전담 | `REQ-INTEG-OBS-01` |
